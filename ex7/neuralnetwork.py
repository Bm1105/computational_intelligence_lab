...
def perceptron():
    ...
    gate = input("Enter logic gate (or type 'custom' for test case): ").lower()
    input_type = input("Data type (binary/bipolar): ").lower()
    n = int(input("Number of inputs (n): "))

    dataset = []
    if gate == "custom":
        xs = [float(input(f"Enter x{i+1}: ")) for i in range(n)]
        target = float(input("Enter target T: "))
    ...
    for epoch in range(1, max_epochs + 1):
        x_headers = " ".join([f"x{i+1:<4}" for i in range(n)])
        dw_headers = " ".join([f"dw{i+1:<4}" for i in range(n)])
        w_headers = " ".join([f"w{i+1:<4}" for i in range(n)])
        ...
        for xs, t in dataset:
            ...
            x_vals = " ".join([f"{val:<4.1f}" for val in xs])
            dw_vals = " ".join([f"{val:<5.1f}" for val in d_ws])
            w_vals = " ".join([f"{val:<5.1f}" for val in weights])

            print(f"{x_vals} | {t:<4.1f} | {yin:<5.2f} | {y:<2} | {dw_vals} | {d_b:<4.1f} | {w_vals} | {bias:.1f}")

        if converged:
            print(f"\nCONVERGED! Final Weights: {weights}, Bias: {bias}")
            break
    else:
        print(f"\nReached {max_epochs} epochs.")
