...
def a_star(graph, start, goal):
    ...
    start_h = graph.heuristics.get(start, 0)
    open_list = [(start_h, 0, start, [start])]
    visited_g = {start: 0}
    ...
    while open_list:
        ...
        h = graph.heuristics.get(curr, 0)
        print(f"after expanding {curr}: g={g}, h={h}, f={f}")
        ...
        for neighbor, weight in graph.adj.get(curr, []):
            ...
            if neighbor not in visited_g or new_g < visited_g[neighbor]:
                ...
                new_h = graph.heuristics.get(neighbor, 0)
                new_f = new_g + new_h

                print(f"  Exploring: {curr}->{neighbor}, new g={new_g}, h={new_h}, f={new_f}")
...
while True:
    ...
    if choice == '1':
        ...
        try:
            ...
            print("Please enter two node names separated by a space.")
    elif choice == '5':
        g.display()
    elif choice == '6':
        s = input("Start node: ")
        target = input("Goal node: ")
        if s not in g.adj or target not in g.adj:
            print("Error: Start or Goal node does not exist.")
        else:
            res_path, res_cost = a_star(g, s, target)
            if res_path:
                print(f"\nPath Found: {' -> '.join(res_path)} | Total Cost: {res_cost}")
            else:
                print("\nNo path found.")
    elif choice == '7':
